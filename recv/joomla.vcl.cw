...
# Pass client's IP in a HTTP header
# Remove this code if you're using Cloudflare or a similar service
...
# More URLs may go here, if needed
        if (req.url ~ "/administrator/" || req.url ~ "ms_http_purge")
        {
                return(pipe);
        }

# Cache static files - add more file extensions if needed
        if (req.url ~ "\.(png|jpg|gif|css|js|swf|flv|ico|xml|txt|pdf|doc|woff|eot|mp[34]|svg|svgz)$"
